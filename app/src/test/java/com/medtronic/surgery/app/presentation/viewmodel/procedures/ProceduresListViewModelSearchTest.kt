...
class ProceduresListViewModelSearchTest {
    ...
    private val mockProcedures = listOf(
        ...
        mockProcedure.copy(
            uuid = "2",
            name = "Lateral Approach to the Hip",
            duration = 75,
            isFavorite = true
        ),
    ...
    fun `filterProcedures should return all procedures when searchQuery is empty and filter is NONE`() {
        val result =
            viewModel.filterProcedures(mockProcedures, "", FilterProcedureType.NONE) { true }
        assertEquals(3, result.size)
    }

    @Test
    fun `filterProcedures should return only favorite procedures`() {
        val result = viewModel.filterProcedures(
            mockProcedures,
            "",
            FilterProcedureType.NONE
        ) { it.isFavorite }
    ...
    fun `filterProcedures should return matching procedures based on search query`() {
        val result =
            viewModel.filterProcedures(mockProcedures, "Lateral", FilterProcedureType.NONE) { true }
        assertEquals(1, result.size)
        assertEquals("Lateral Approach to the Hip", result.first().name)
    }
    ...
    fun `filterProcedures should return empty when search query has no match`() {
        val result =
            viewModel.filterProcedures(mockProcedures, "Brain", FilterProcedureType.NONE) { true }
    ...
    fun `filterProcedures should sort by duration ascending`() {
        val result = viewModel.filterProcedures(
            mockProcedures,
            "",
            FilterProcedureType.DURATION_ASCENDING
        ) { true }
        assertEquals(
            listOf("Lateral Approach to the Hip", "IMA Harvesting", "Cemented Hip Cup"),
            result.map { it.name })
    }
    ...
    fun `filterProcedures should sort by duration descending`() {
        val result = viewModel.filterProcedures(
            mockProcedures,
            "",
            FilterProcedureType.DURATION_DESCENDING
        ) { true }
        assertEquals(
            listOf("Cemented Hip Cup", "IMA Harvesting", "Lateral Approach to the Hip"),
            result.map { it.name })
    }
    ...
    fun `filterProcedures should sort alphabetically ascending`() {
        val result = viewModel.filterProcedures(
            mockProcedures,
            "",
            FilterProcedureType.ALPHABETICAL_ASCENDING
        ) { true }
        assertEquals(
            listOf("Cemented Hip Cup", "IMA Harvesting", "Lateral Approach to the Hip"),
            result.map { it.name })
    ...
    fun `filterProcedures should sort alphabetically descending`() {
        val result = viewModel.filterProcedures(
            mockProcedures,
            "",
            FilterProcedureType.ALPHABETICAL_DESCENDING
        ) { true }
        assertEquals(
            listOf("Lateral Approach to the Hip", "IMA Harvesting", "Cemented Hip Cup"),
            result.map { it.name })
